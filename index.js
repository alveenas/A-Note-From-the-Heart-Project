...
app.post("/submit", async (req, res) => {
  try {
    ...
    // Uses columns: title, message, reportcount, hidden
    await pool.query(
...
app.post("/report", async (req, res) => {
  try {
    ...
    if (u.rows[0].reportcount >= 3) {
      await pool.query(`UPDATE notes SET hidden = TRUE WHERE id = $1`, [noteId]);
    }
...
/* Serve admin page at /admin (and /admin.html still works because static) */
app.get("/admin", (req, res) => {
  res.sendFile(path.join(__dirname, "public", "admin.html"));
});

/* Admin API: fetch notes + feedback */
...
app.post("/admin/toggleHidden", adminAuth, async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE notes SET hidden = $1 WHERE id = $2`, [!!hidden, noteId]);
    res.sendStatus(200);
  } catch (err) {
...
/* Admin: reset reportcount (optional button behavior) */
app.post("/admin/resetReports", adminAuth, async (req, res) => {
  try {
    const { noteId } = req.body;
    if (!noteId) return res.status(400).send("Missing noteId");

    await pool.query(`UPDATE notes SET reportcount = 0 WHERE id = $1`, [noteId]);
    res.sendStatus(200);
  } catch (err) {
